...
func (i *Interp) SetPcapByte(data []byte) *Interp {
	if data == nil || len(data) == 0 {
		return i
	}
	i.PcapData = make([]byte, len(data))
	copy(i.PcapData, data)
	...
	compilerOpts = append(compilerOpts, gojq.WithModuleLoader(loadModule{
		...
		load: func(name string) (*gojq.Query, error) {
			...
			if q.Term != nil || q.Op != gojq.Operator(0) {
				...
				iter := gc.RunWithContext(ctx, nil)
				var vs []any
				for {
					v, ok := iter.Next()
					if !ok {
...
func (i *Interp) EvalFuncWithoutArg(ctx context.Context, c any, name string, opts EvalOpts) (gojq.Iter, error) {
	argExpr := ""
	...
	iter, err := i.Eval(ctx, trampolineInput, trampolineExpr, opts) // todo 核心入口
	if err != nil {
		return nil, err
	}
	return iter, nil

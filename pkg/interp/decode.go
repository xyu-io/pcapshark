...
// 获取、组装解析器的group,format和依赖文件信息（embed注入）
...
func (i *Interp) _decode(c any, format string, opts decodeOpts) any {
	...
	decodeGroup, err := i.Registry.Group(formatName) // 根据名称获取解码器 --> pcap
	if err != nil {
		return err
	}
	// decode 入口 - 一次 ,需要先完成RegisterFormat
	deValue, formatOut, err := decode.Decode(i.EvalInstance.Ctx, bv.br, decodeGroup,
		decode.Options{
			IsRoot:      true,
			FillGaps:    true,
			Force:       opts.Force,
			...
			ParseOptsFn: func(init any) any { // decodeGroup.DefaultInArg 不为nil使用
	...
	var out = makeDecodeValueOut(deValue, decodeValueValue, formatOutMap)
	//normalValue, err := toValue(func() (*Options, error) { return &Options{}, nil }, out)
	//if err != nil {
	//	fmt.Println(err)
	//}
	//fmt.Printf(">> %+v", normalValue)
	return out
}

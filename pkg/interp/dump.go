...
import (
	...
	"github.com/xyu-io/pcapshark/internal/ansi"
	"github.com/xyu-io/pcapshark/internal/asciiwriter"
	"github.com/xyu-io/pcapshark/internal/bitiox"
	"github.com/xyu-io/pcapshark/internal/columnwriter"
...
func dumpEx(v *decode.Value, ctx *dumpCtx, depth int, rootV *decode.Value, rootDepth int, addrWidth int, ps PcapSct) error {
	opts := ctx.opts
	cw := ctx.cw
	buf := ctx.buf
	...
	if isInArray {
		////todo stdout->yaml cfmt(colField, "%s%s%s", deco.Index.F("["), deco.Number.F(strconv.Itoa(v.Index)), deco.Index.F("]"))
		cfmt(colField, "%s%s%s", deco.Index.F("index"), deco.Index.F("_"), deco.Number.F(strconv.Itoa(v.Index)))
	}
	...
	case *decode.Compound:
		if vv.IsArray {
			//todo stdout->yaml cfmt(colField, "[#]%s%s:%s%s", deco.Index.F("["), deco.Number.F("0"), deco.Number.F(strconv.Itoa(len(vv.Children))), deco.Index.F("]"))
			cfmt(colField, "[#]_%s_%s", deco.Number.F("0"), deco.Number.F(strconv.Itoa(len(vv.Children))))
		} else {
			//todo stdout->yaml cfmt(colField, "[#]%s", deco.Object.F("{}"))
	...
	case scalar.Scalarable:
		...
		if sym == nil {
			...
			//todo stdout->yaml cfmt(colField, "[#] (%s)", deco.ValueColor(actual).F(previewValue(actual, df, opts)))
		}
	...
	if isCompound {
		...
		if desc != "" {
			//todo stdout->yaml cfmt(colField, " (%s)", deco.Value.F(desc))
		}
	}

	if v.Format != nil {
		//todo stdout->yaml cfmt(colField, " (%s)", deco.Value.F(v.Format.Name))
	}
	valueErr := v.Err

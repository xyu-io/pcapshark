...
import (
	...
	"github.com/xyu-io/pcapshark/internal/aheadreadseeker"
	"github.com/xyu-io/pcapshark/internal/bitiox"
	"github.com/xyu-io/pcapshark/internal/ctxreadseeker"
	"github.com/xyu-io/pcapshark/internal/gojqx"
	"github.com/xyu-io/pcapshark/internal/iox"
	"github.com/xyu-io/pcapshark/internal/progressreadseeker"
...
// 获取文件操作对象 - 操作文件，非配置等其他文件
func (i *Interp) getF(c any) (fs.File, error) {
	var path string
	var f fs.File
	var err error
...
func (i *Interp) _open(c any) any {
	if i.EvalInstance.IsCompleting { // TODO: have dummy values for each type for completion?
	...
	if len(i.PcapData) > 0 {
		...
		bEnd = int64(len(buf))
		i.PcapData = make([]byte, 0)
	} else { // 采用文件读取或者Stdin方式读取文件数据
		if f, err = i.getF(c); err != nil {
			return err
		}
		// 优先采用原始内置文件读取方式

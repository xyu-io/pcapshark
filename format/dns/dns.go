...
func fieldDecodeLabel(d *decode.D, pointerOffset int64, name string) {
	...
	d.FieldStruct(name, func(d *decode.D) {
		...
		d.FieldArray("labels", func(d *decode.D) {
			seenTermintor := false
			for !seenTermintor {
				//todo stdout->yaml d.FieldStruct("label", func(d *decode.D) {
				d.FieldStruct("", func(d *decode.D) {
					d.FieldValueStr("type_name", "label")
...
func dnsDecodeRR(d *decode.D, pointerOffset int64, resp bool, count uint64, name string, structName string) {
	d.FieldArray(name, func(d *decode.D) {
		for i := uint64(0); i < count; i++ {
			//todo stdout->yaml d.FieldStruct(structName, func(d *decode.D) {
			d.FieldStruct("", func(d *decode.D) {
				d.FieldValueStr("type_name", structName)
...
func dnsDecode(d *decode.D, hasLengthHeader bool) any {
	...
	dnsDecodeRR(d, pointerOffset, false, qdCount, "questions", "")
	dnsDecodeRR(d, pointerOffset, true, anCount, "answers", "")
	dnsDecodeRR(d, pointerOffset, true, nsCount, "nameservers", "")
	dnsDecodeRR(d, pointerOffset, true, arCount, "additionals", "")

...
import (
	...
	"github.com/xyu-io/pcapshark/format/tls/ciphersuites"
	"github.com/xyu-io/pcapshark/format/tls/keylog"
	"github.com/xyu-io/pcapshark/format/tls/rezlib"
	"github.com/xyu-io/pcapshark/format/tls/tlsdecrypt"
	...
	d.FramedFn(int64(length)*8, func(d *decode.D) {
		...
		case extensionServerName:
			...
			d.FieldArray("server_names", func(d *decode.D) {
				d.FramedFn(int64(serverNameLength)*8, func(d *decode.D) {
					for !d.End() {
						//todo stdout->yaml d.FieldStruct("server_name", func(d *decode.D) {
						d.FieldStruct("", func(d *decode.D) {
							d.FieldValueStr("type_name", "server_name")
							d.FieldU8("type") // TODO:
							length := d.FieldU16("length")
		...
		case extensionApplicationLayerProtocolNegotiation:
			...
			d.FieldArray("protocols", func(d *decode.D) {
				d.FramedFn(int64(protocolsLength)*8, func(d *decode.D) {
					for !d.End() {
						//todo stdout->yaml d.FieldStruct("protocol", func(d *decode.D) {
						d.FieldStruct("", func(d *decode.D) {
							d.FieldValueStr("type_name", "protocol")
		...
		case extensionSignatureAlgorithms:
			...
			d.FieldArray("signature_algorithms", func(d *decode.D) {
				d.FramedFn(int64(protocolsLength)*8, func(d *decode.D) {
					for !d.End() {
						//todo stdout->yaml d.FieldStruct("signature_algorithm", func(d *decode.D) {
						d.FieldStruct("", func(d *decode.D) {
							d.FieldValueStr("type_name", "signature_algorithm")
			...
			if d.BitsLeft() > 0 {
				...
				d.FramedFn(int64(extensionsLength)*8, func(d *decode.D) {
					d.FieldArray("extensions", func(d *decode.D) {
						for !d.End() {
							//todo stdout->yaml d.FieldStruct("extension", decodeTLSExtension)
							d.FieldStruct("", decodeTLSExtension)
						}
					})
				})
			}
...
func decodeTLS(d *decode.D) any {
	...
	d.FieldArray("records", func(d *decode.D) {
		for !d.End() {
			////todo stdout->yaml  d.FieldStruct("record", func(d *decode.D) {
			d.FieldStruct("", func(d *decode.D) {
				d.FieldValueStr("type_name", "record")
